...
class TestGetStructuredChatModel:
    ...
    def test_anthropic_uses_native_structured_output(self, mock_caps) -> None:
        """Anthropic with structured output support uses constrained decoding."""
...
class TestApplyStructuredOutputKwargs:
    ...
    @patch("llm.base.get_model_capabilities")
    def test_openai_adds_text_format(self, mock_caps) -> None:
        """OpenAI provider adds text.format via Responses API."""
        mock_caps.return_value = {"structured_output": True}
        client = _make_client(
            provider="openai",
            _output_schema={"schema": {"type": "object"}},
    ...
    @patch("llm.base.get_model_capabilities")
    def test_openai_with_text_key_adds_format_inside(self, mock_caps) -> None:
    ...
    @patch("llm.base.get_model_capabilities")
    def test_openai_no_schema_no_format(self, mock_caps) -> None:
    ...
    @patch("llm.base.get_model_capabilities")
    def test_openai_no_structured_output_suppresses_format(self, mock_caps) -> None:
        """OpenAI model without structured output support adds nothing."""
        mock_caps.return_value = {"structured_output": False}
        client = _make_client(
            provider="openai",
            _output_schema={"schema": {"type": "object"}},
    ...
    def test_google_suppresses_schema_when_thinking_active(self) -> None:
        """Google with thinking_config active skips response_schema."""
        client = _make_client(
            provider="google",
            _output_schema={"schema": {"type": "object", "properties": {}}},
        )
        kwargs: dict[str, Any] = {"thinking_config": {"thinking_budget": 4096}}
        client._apply_structured_output_kwargs(kwargs)
        assert "response_mime_type" not in kwargs
