...
- CONFIG_DIR: config/defaults/ directory (YAML defaults)
- PROMPTS_DIR: llm/resources/prompts/ directory (prompt templates)
- SCHEMAS_DIR: llm/resources/schemas/ directory (JSON schemas)
...
CONFIG_DIR = Path(__file__).resolve().parent / "defaults"
PROMPTS_DIR = PROJECT_ROOT / "llm" / "resources" / "prompts"
SCHEMAS_DIR = PROJECT_ROOT / "llm" / "resources" / "schemas"
...
class ConfigLoader:
    """
    Lightweight loader for YAML configs residing under config/defaults/.

    This class loads configuration files for image processing, concurrency,
    and model settings from the config/defaults/ directory.
    ...
    def apply_model_overrides(self, overrides: dict[str, Any]) -> None:
        ...
        overrides without editing ``config/defaults/model.yaml``.
        """
