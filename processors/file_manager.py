...
def sanitize_omml_xml(omml_markup: str) -> str:
    ...
    try:
        ...
        if 'groupChr' in str(e):
            # Try to fix unclosed groupChrPr tags by ensuring proper closure
            fixed_markup = re.sub(
                r'(<m:groupChrPr[^>]*>)(?!.*?</m:groupChrPr>)(.*?)(</m:groupChr>)',
                r'\1\2</m:groupChrPr>\3',
                omml_markup,
                flags=re.DOTALL
            )
...
def add_math_to_paragraph(paragraph, latex_code: str) -> None:
    ...
    try:
        ...
        try:
            # Remove complex grouping characters that often cause issues
            simplified_latex = latex_code.replace('\\land', '\\wedge')
            simplified_latex = simplified_latex.replace('\\lor', '\\vee')
            
            if simplified_latex != latex_code:
                logger.info("Attempting conversion with simplified LaTeX")
