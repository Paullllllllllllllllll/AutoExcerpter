...
def simplify_problematic_latex(latex_code: str) -> tuple[str, list[str]]:
    ...
    Returns:
        Tuple of (simplified_latex, list of applied simplifications).
    """
...
def sanitize_omml_xml(omml_markup: str) -> str:
    ...
    This addresses known bugs where groupChr elements have malformed XML structure.
    
    Args:
        omml_markup: The OMML markup string to sanitize.
        
    Returns:
        Sanitized OMML markup string.
    """
    try:
...
def add_math_to_paragraph(paragraph, latex_code: str) -> None:
    ...
    1. Try full LaTeX -> MathML -> OMML -> Word conversion
    2. If XML parsing fails, attempt to sanitize the OMML
    3. If sanitization fails, try simplified LaTeX (remove complex grouping)
    4. Final fallback: display as formatted text
